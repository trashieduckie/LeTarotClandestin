...
<html>
<head> <title> Editer une partie </title> </head>
<body>
<?php
...
?>
<?php
	if(isset($_SESSION['nom']) && isset($_GET['idpartie']))
	{
		
		echo "Si vous lisez ça c'est que vous vous êtes trompés dans la saisie de la partie ". $_GET['idpartie'] .". Vous êtes officiellement une merde.";	

	}
	echo "<form action = \"editerPartie.php?idpartie=".$_GET['idpartie']."\" method=\"post\">";
?>
  <p>
     	<BR>
     	Session :
...
<?php
	if(isset($_POST['Button']) && isset($_SESSION['nom']) && isset($_GET['idpartie']))
	{
		...
		)
		{
			...
			else
			{
				...
				else
				{
					...
					DeletePartie($_GET['idpartie']);
					// On commence les insertions, motherfucker
					InsertPartieWithID($IDPartie, $NomSession,$ScorePartie,$NbBoutsPartie,$NomPreneur,$NomEquipier,$Contrat,$Chelem,$ChelemOK,$NomPetitAuBout,$PetitAuBoutReussi);
				...
				echo "<BR>Partie ". $_GET['idpartie'] ." modifiée. Bravo.<BR>";
			}
